...
int WINAPI wWinMain(HINSTANCE,HINSTANCE,PWSTR,int){std::array<wchar_t,32768> raw{};const auto length=GetModuleFileNameW(nullptr,raw.data(),static_cast<DWORD>(raw.size()));if(!length||length==raw.size())return 1;const std::filesystem::path self(std::wstring(raw.data(),length));std::ifstream input(self,std::ios::binary|std::ios::ate);Footer footer{};input.seekg(-static_cast<std::streamoff>(sizeof(footer)),std::ios::end);input.read(reinterpret_cast<char*>(&footer),sizeof(footer));if(!input||std::string_view(footer.magic,8)!="WCHBNDL1")return 1;PWSTR base{};const auto folder=footer.mode==2?FOLDERID_ProgramFiles:FOLDERID_LocalAppData;if(FAILED(SHGetKnownFolderPath(folder,0,nullptr,&base)))return 1;std::filesystem::path target=base;CoTaskMemFree(base);target/=L"Winchisel";target/=footer.mode==2?L"":L"portable";target/=footer.mode==2?L"":std::to_wstring(footer.table_offset);if(!unpack(self,target))return 1;return start(target/L"Winchisel.exe")?0:1;}
