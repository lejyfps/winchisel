...
bool unpack(std::filesystem::path const& self,std::filesystem::path const& target,std::vector<std::wstring>* installed_paths=nullptr) {
    ...
    DECOMPRESSOR_HANDLE decompressor{};if(!CreateDecompressor(COMPRESS_ALGORITHM_LZMS,nullptr,&decompressor))return false;std::error_code error;std::filesystem::create_directories(target,error);if(error){CloseDecompressor(decompressor);return false;}
