...
- **Change history:** every applied tweak records its before-state; undo single entries or all of them from Change history in the sidebar footer (or the title-bar button) — best-effort, the restore point stays the safety net
...
Open Change history (sidebar footer, above Settings) and undo the entry — or Undo all. Alternatively flip the toggle back, use the per-tweak Default button, apply a Defaults profile, or restore the system restore point you created first.
