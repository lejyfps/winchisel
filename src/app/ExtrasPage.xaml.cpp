...
ExtrasPage::ExtrasPage(){InitializeComponent();load_states();}
...
void ExtrasPage::ToggleChanged(Windows::Foundation::IInspectable const& sender,Microsoft::UI::Xaml::RoutedEventArgs const&){if(loading_)return;auto toggle=sender.as<muxc::ToggleSwitch>();bool enabled=toggle.IsOn(),ok=true;
 ...
 else {loading_=true;toggle.IsOn(!enabled);loading_=false;show_result(false,L"This command-based action is being ported next.");return;}
 if(!ok){load_states();show_result(false,L"Could not apply the setting.");}else show_result(true,L"Setting applied.");
}
void ExtrasPage::PowerPlanClick(Windows::Foundation::IInspectable const&,Microsoft::UI::Xaml::RoutedEventArgs const&){show_result(false,L"Power plan import is being ported next.");}
}
