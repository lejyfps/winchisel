...
void ProcessesPage::render_processes(){
    ...
    std::vector<std::uint32_t> current_pids;current_pids.reserve(visible.size());for(auto row:visible)current_pids.push_back(row->pid);
    if(current_pids==rendered_pids_&&ProcessList().Items().Size()==visible.size()){
    ...
    rendered_pids_=std::move(current_pids);ProcessList().Items().Clear();
