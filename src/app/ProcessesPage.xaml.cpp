...
void ProcessesPage::load_processes(){
    rows_.clear();auto current_system=system_cpu_time();auto system_delta=current_system>previous_system_time_?current_system-previous_system_time_:0;
    std::unordered_map<std::uint32_t,std::uint64_t> current_times;
    HANDLE snapshot=CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS,0);if(snapshot==INVALID_HANDLE_VALUE){StatusText().Text(L"Process scan failed.");return;}
    ...
    if(Process32FirstW(snapshot,&entry))do{
        ...
        if(process){
            FILETIME created{},exited{},kernel{},user{};if(GetProcessTimes(process,&created,&exited,&kernel,&user)){auto value=file_time(kernel)+file_time(user);current_times[row.pid]=value;if(auto old=previous_process_times_.find(row.pid);old!=previous_process_times_.end()&&system_delta&&value>=old->second)row.cpu=100.0*static_cast<double>(value-old->second)/static_cast<double>(system_delta);}
    ...
    previous_process_times_=std::move(current_times);previous_system_time_=current_system;
    std::unordered_set<std::uint32_t> live;for(auto const& row:rows_)live.insert(row.pid);for(auto it=expanded_.begin();it!=expanded_.end();)if(!live.contains(*it))it=expanded_.erase(it);else++it;
    render_processes();
}

void ProcessesPage::render_processes(){
    auto scroll=find_scroll_viewer(ProcessList());auto vertical_offset=scroll?scroll.VerticalOffset():0.0;restoring_selection_=true;ProcessList().Items().Clear();auto mode=FilterBox().SelectedIndex();DWORD own_session{};ProcessIdToSessionId(GetCurrentProcessId(),&own_session);
    ...
    muxc::Grid selected_grid{nullptr};for(auto row:visible){
        ...
        muxc::StackPanel pid_panel;pid_panel.Orientation(muxc::Orientation::Horizontal);pid_panel.Spacing(4);auto pid=row->pid;if(row->has_children){muxc::Button expand;expand.Padding({4,0,4,0});expand.MinWidth(24);expand.Content(winrt::box_value(expanded_.contains(row->pid)?L"−":L"+"));auto weak=get_weak();expand.Click([weak,pid](auto&&,auto&&){if(auto self=weak.get()){if(!self->expanded_.insert(pid).second)self->expanded_.erase(pid);self->DispatcherQueue().TryEnqueue([weak]{if(auto page=weak.get())page->render_processes();});}});pid_panel.Children().Append(expand);}else{muxc::Border spacer;spacer.Width(24);pid_panel.Children().Append(spacer);}muxc::TextBlock pid_text;pid_text.Text(std::to_wstring(row->pid));pid_text.VerticalAlignment(mux::VerticalAlignment::Center);pid_panel.Children().Append(pid_text);grid.Children().Append(pid_panel);
        ...
        wchar_t cpu[24]{};swprintf_s(cpu,L"%.1f%%",row->cpu);add(row->name,1,row->depth*16.0);add(cpu,2);add(row->priority,3);add(row->affinity,4);add(row->status,5);add_process_menu(grid,*row);ProcessList().Items().Append(grid);if(row->pid==selected_pid_)selected_grid=grid;
    }
...
bool ProcessesPage::set_affinity(std::uint32_t pid,DWORD_PTR mask){HANDLE process=OpenProcess(PROCESS_SET_INFORMATION|PROCESS_QUERY_LIMITED_INFORMATION,false,pid);if(!process)return false;DWORD_PTR current{},system{};bool ok=GetProcessAffinityMask(process,&current,&system)&&mask&&(mask&system)&&SetProcessAffinityMask(process,mask&system);CloseHandle(process);if(ok){if(open_overlays_)refresh_pending_=true;else load_processes();}else StatusText().Text(L"Could not change process affinity.");return ok;}
...
bool ProcessesPage::set_always(std::wstring const& name,wchar_t const* value_name,DWORD value){std::wstring exe=name.ends_with(L".exe")?name:name+L".exe";std::wstring path=L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\"+exe+L"\\PerfOptions";HKEY key{};if(RegCreateKeyExW(HKEY_LOCAL_MACHINE,path.c_str(),0,nullptr,0,KEY_SET_VALUE,nullptr,&key,nullptr)!=ERROR_SUCCESS)return false;auto result=RegSetValueExW(key,value_name,0,REG_DWORD,reinterpret_cast<BYTE*>(&value),sizeof(value));RegCloseKey(key);return result==ERROR_SUCCESS;}
bool ProcessesPage::remove_always(std::wstring const& name,wchar_t const* value_name){std::wstring exe=name.ends_with(L".exe")?name:name+L".exe";std::wstring base=L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\"+exe;std::wstring path=base+L"\\PerfOptions";HKEY key{};if(RegOpenKeyExW(HKEY_LOCAL_MACHINE,path.c_str(),0,KEY_QUERY_VALUE|KEY_SET_VALUE,&key)!=ERROR_SUCCESS)return !read_always(name,value_name).has_value();const auto removed=RegDeleteValueW(key,value_name);DWORD values{},subkeys{};const bool empty=RegQueryInfoKeyW(key,nullptr,nullptr,nullptr,&subkeys,nullptr,nullptr,&values,nullptr,nullptr,nullptr,nullptr)==ERROR_SUCCESS&&!values&&!subkeys;RegCloseKey(key);if(empty)RegDeleteKeyW(HKEY_LOCAL_MACHINE,path.c_str());HKEY parent{};if(RegOpenKeyExW(HKEY_LOCAL_MACHINE,base.c_str(),0,KEY_QUERY_VALUE,&parent)==ERROR_SUCCESS){if(RegQueryInfoKeyW(parent,nullptr,nullptr,nullptr,&subkeys,nullptr,nullptr,&values,nullptr,nullptr,nullptr,nullptr)==ERROR_SUCCESS&&!values&&!subkeys){RegCloseKey(parent);RegDeleteKeyW(HKEY_LOCAL_MACHINE,base.c_str());}else RegCloseKey(parent);}return removed==ERROR_SUCCESS||removed==ERROR_FILE_NOT_FOUND;}
std::optional<DWORD> ProcessesPage::read_always(std::wstring const& name,wchar_t const* value_name){std::wstring exe=name.ends_with(L".exe")?name:name+L".exe";std::wstring path=L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\"+exe+L"\\PerfOptions";DWORD value{},size=sizeof(value),type{};if(RegGetValueW(HKEY_LOCAL_MACHINE,path.c_str(),value_name,RRF_RT_REG_DWORD,&type,&value,&size)==ERROR_SUCCESS)return value;return std::nullopt;}
...
void ProcessesPage::add_process_menu(mux::FrameworkElement const& element,ProcessRow const& row){
    ...
    auto affinity=submenu(L"Affinity");add(affinity,L"Edit…",false,[this,pid,name](auto&&,auto&&){edit_affinity(pid,name);});
