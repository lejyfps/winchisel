...
void ProcessesPage::add_process_menu(mux::FrameworkElement const& element,ProcessRow const& row,MenuSnapshot const& snap){
    ...
    auto preset_checked=[&](int preset){return snap.affinity_ok&&snap.affinity_current==static_cast<DWORD_PTR>(winchisel::core::affinity_mask(snap.affinity_system,preset));};
    add(affinity,L"All cores",preset_checked(0),[mode](auto&&,auto&&){mode(0);});add(affinity,L"Even logical CPUs",preset_checked(1),[mode](auto&&,auto&&){mode(1);});add(affinity,L"Odd logical CPUs",preset_checked(2),[mode](auto&&,auto&&){mode(2);});add(affinity,L"First half",preset_checked(3),[mode](auto&&,auto&&){mode(3);});add(affinity,L"Second half",preset_checked(4),[mode](auto&&,auto&&){mode(4);});element.ContextFlyout(menu);
}
