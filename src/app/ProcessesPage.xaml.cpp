...
bool ProcessesPage::set_io_priority(std::uint32_t pid,std::uint32_t value){using Fn=BOOL(WINAPI*)(HANDLE,int,LPVOID,DWORD);auto fn=reinterpret_cast<Fn>(GetProcAddress(GetModuleHandleW(L"kernel32.dll"),"SetProcessInformation"));HANDLE process=OpenProcess(PROCESS_SET_INFORMATION|PROCESS_QUERY_LIMITED_INFORMATION,false,pid);bool ok=fn&&process&&fn(process,33,&value,sizeof(value));if(process)CloseHandle(process);if(ok){if(open_overlays_)refresh_pending_=true;else load_processes();}else StatusText().Text(L"Could not change I/O priority.");return ok;}
...
void ProcessesPage::add_process_menu(mux::FrameworkElement const& element,ProcessRow const& row){
    ...
    auto io_saved=read_always(name,L"IoPriority");auto io_always=submenu(L"I/O priority · Always");add(io_always,L"Low",io_saved&&*io_saved==1,[this,pid,name](auto&&,auto&&){if(set_always(name,L"IoPriority",1))set_io_priority(pid,1);else StatusText().Text(L"Could not save permanent I/O priority.");});add(io_always,L"Normal",io_saved&&*io_saved==2,[this,pid,name](auto&&,auto&&){if(set_always(name,L"IoPriority",2))set_io_priority(pid,2);else StatusText().Text(L"Could not save permanent I/O priority.");});
