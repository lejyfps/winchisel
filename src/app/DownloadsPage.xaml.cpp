...
void DownloadsPage::render_items() {
 ...
 for(int category_index=0;category_index<16;++category_index){auto category=static_cast<winchisel::core::DownloadCategory>(category_index);auto list=Controls::ListView();list.SelectionMode(Controls::ListViewSelectionMode::Multiple);list.HorizontalContentAlignment(HorizontalAlignment::Stretch);list.SelectionChanged({this,&DownloadsPage::Items_SelectionChanged});std::size_t category_count{};
  ...
  if(category_count){auto expander=Controls::Expander();expander.Header(box_value(winchisel::ui::tr(winrt::to_hstring(winchisel::core::download_category_name(category)))+L" ("+to_hstring(category_count)+L")"));expander.HorizontalAlignment(HorizontalAlignment::Stretch);expander.HorizontalContentAlignment(HorizontalAlignment::Stretch);expander.IsExpanded(!query.empty()||category_index==0);expander.Content(list);category_lists_.push_back(list);Groups().Children().Append(expander);}
 }
...
void DownloadsPage::update_actions(){std::uint32_t count{};for(auto const& list:category_lists_)for(auto const& value:list.SelectedItems())if(auto row=value.try_as<Controls::ListViewItem>()){auto index=unbox_value<std::uint64_t>(row.Tag());if(index<catalog_.size()&&!catalog_[index].winget_ids.empty())++count;}InstallButton().IsEnabled(count&&operation_==Operation::none);InstallButton().Content(box_value(count?winchisel::ui::tr(L"Install")+L" ("+to_hstring(count)+L")":winchisel::ui::tr(L"Install")));}
