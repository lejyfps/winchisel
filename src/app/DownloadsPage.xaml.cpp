...
void DownloadsPage::apply_filter() {
 auto query=lower(to_string(Search().Text())); auto filter=Filter().SelectedIndex(); std::size_t visible{}; bool first_visible=true;
 for(auto const& child:Groups().Children()){auto expander=child.try_as<Controls::Expander>();if(!expander)continue;auto list=expander.Content().try_as<Controls::ListView>();if(!list)continue;std::size_t count{};
  ...
  for(auto const& value:list.Items()){auto row=value.try_as<Controls::ListViewItem>();if(!row)continue;const auto index=unbox_value<std::uint64_t>(row.Tag());bool show=index<catalog_.size()&&index<search_index_.size();if(show){if(!query.empty()&&search_index_[static_cast<std::size_t>(index)].find(query)==std::string::npos)show=false;if(show&&filter==1&&!installed_[static_cast<std::size_t>(index)])show=false;if(show&&filter==2&&installed_[static_cast<std::size_t>(index)])show=false;}
   row.Visibility(show?Visibility::Visible:Visibility::Collapsed);if(show)++count;else if(row.IsSelected())deselect.push_back(value);}
  ...
  auto base=unbox_value_or<hstring>(expander.Tag(),hstring{});
  expander.Header(box_value(winrt::hstring{std::wstring(base)+L" ("+std::to_wstring(count)+L")"}));
  expander.Visibility(count?Visibility::Visible:Visibility::Collapsed);
  if(count)expander.IsExpanded(!query.empty()||first_visible);
...
void DownloadsPage::update_actions(){std::uint32_t count{};for(auto const& list:category_lists_)for(auto const& value:list.SelectedItems())if(auto row=value.try_as<Controls::ListViewItem>()){if(row.Visibility()!=Visibility::Visible)continue;auto index=unbox_value<std::uint64_t>(row.Tag());if(index<catalog_.size()&&!catalog_[index].winget_ids.empty())++count;}InstallButton().IsEnabled(count&&operation_==Operation::none);InstallButton().Content(box_value(count?winrt::hstring{std::wstring(winchisel::ui::tr(L"Install"))+L" ("+std::to_wstring(count)+L")"}:winchisel::ui::tr(L"Install")));}
