...
DownloadsPage::DownloadsPage() {
 ...
 timer_=DispatcherTimer(); timer_.Interval(std::chrono::milliseconds(120)); timer_token_=timer_.Tick([this](auto&&,auto&&){ poll_worker(); }); ui_ready_=true; start_scan();
}
DownloadsPage::~DownloadsPage() { timer_.Stop(); timer_.Tick(timer_token_); }
...
void DownloadsPage::poll_worker() {
 if(operation_==Operation::scan) {
  ...
  if(result){Notice().Message(to_hstring(std::to_string(result->succeeded)+" succeeded, "+std::to_string(result->failed)+" failed."));Notice().Severity(result->failed?Controls::InfoBarSeverity::Warning:Controls::InfoBarSeverity::Success);}else{Notice().Message(to_hstring(result.error().detail));Notice().Severity(Controls::InfoBarSeverity::Error);} Notice().IsOpen(true); update_actions(); start_scan(true,false);
 } else timer_.Stop();
}
void DownloadsPage::render_items() {
 ...
 for(int category_index=0;category_index<16;++category_index){auto category=static_cast<winchisel::core::DownloadCategory>(category_index);auto list=Controls::ListView();list.SelectionMode(Controls::ListViewSelectionMode::Multiple);list.HorizontalContentAlignment(HorizontalAlignment::Stretch);list.SelectionChanged({this,&DownloadsPage::Items_SelectionChanged});std::size_t category_count{};
  for(std::size_t index{};index<catalog_.size();++index){auto const& item=catalog_[index];if(item.category!=category)continue;auto category_name=winchisel::core::download_category_name(category);auto searchable=lower(std::string(item.name)+" "+std::string(category_name)+" "+std::string(item.winget_ids));if(!query.empty()&&searchable.find(query)==std::string::npos)continue;if(filter==1&&!installed_[index])continue;if(filter==2&&installed_[index])continue;
   auto row=Controls::ListViewItem();row.Tag(box_value(static_cast<std::uint64_t>(index)));row.HorizontalContentAlignment(HorizontalAlignment::Stretch);row.Background(resources.Lookup(box_value(L"CardBackgroundFillColorDefaultBrush")).try_as<Media::Brush>());row.BorderBrush(resources.Lookup(box_value(L"CardStrokeColorDefaultBrush")).try_as<Media::Brush>());row.BorderThickness({1,1,1,1});row.CornerRadius({4,4,4,4});row.Padding({12,10,12,10});row.Margin({0,0,0,8});
   auto grid=Controls::Grid();grid.ColumnDefinitions().Append(Controls::ColumnDefinition());auto trailing=Controls::ColumnDefinition();trailing.Width({0,GridUnitType::Auto});grid.ColumnDefinitions().Append(trailing);auto text=Controls::StackPanel();auto title=Controls::TextBlock();title.Text(to_hstring(item.name));title.Style(resources.Lookup(box_value(L"BodyStrongTextBlockStyle")).try_as<Microsoft::UI::Xaml::Style>());text.Children().Append(title);auto detail=Controls::TextBlock();detail.Text(to_hstring(item.winget_ids.empty()?"Website install":std::string(item.winget_ids)));detail.Foreground(secondary);detail.TextWrapping(TextWrapping::Wrap);detail.Style(resources.Lookup(box_value(L"CaptionTextBlockStyle")).try_as<Microsoft::UI::Xaml::Style>());text.Children().Append(detail);grid.Children().Append(text);
   auto actions=Controls::StackPanel();actions.Orientation(Controls::Orientation::Horizontal);actions.Spacing(12);auto status=Controls::TextBlock();status.Text(installed_[index]?L"Installed":L"Not installed");status.Foreground(installed_[index]?success:secondary);status.VerticalAlignment(VerticalAlignment::Center);actions.Children().Append(status);auto website=Controls::HyperlinkButton();website.Content(box_value(L"Website"));website.Tag(box_value(to_hstring(item.website_url)));website.Click({this,&DownloadsPage::Website_Click});actions.Children().Append(website);Controls::Grid::SetColumn(actions,1);grid.Children().Append(actions);row.Content(grid);list.Items().Append(row);++category_count;++visible;
  }
  if(category_count){auto expander=Controls::Expander();expander.Header(box_value(to_hstring(std::string(winchisel::core::download_category_name(category))+" ("+std::to_string(category_count)+")")));expander.HorizontalAlignment(HorizontalAlignment::Stretch);expander.HorizontalContentAlignment(HorizontalAlignment::Stretch);expander.IsExpanded(!query.empty()||category_index==0);expander.Content(list);category_lists_.push_back(list);Groups().Children().Append(expander);}
 }
...
void DownloadsPage::Refresh_Click(IInspectable const&,RoutedEventArgs const&){if(ui_ready_)start_scan(true);} void DownloadsPage::Install_Click(IInspectable const&,RoutedEventArgs const&){if(ui_ready_)confirm_install();} void DownloadsPage::Search_TextChanged(IInspectable const&,Controls::AutoSuggestBoxTextChangedEventArgs const&){if(ui_ready_&&operation_==Operation::none)render_items();} void DownloadsPage::Filter_SelectionChanged(IInspectable const&,Controls::SelectionChangedEventArgs const&){if(ui_ready_&&operation_==Operation::none)render_items();} void DownloadsPage::Items_SelectionChanged(IInspectable const&,Controls::SelectionChangedEventArgs const&){if(ui_ready_)update_actions();}
void DownloadsPage::Website_Click(IInspectable const& sender,RoutedEventArgs const&){if(auto button=sender.try_as<Controls::HyperlinkButton>()){auto url=unbox_value<hstring>(button.Tag());ShellExecuteW(nullptr,L"open",url.c_str(),nullptr,nullptr,SW_SHOWNORMAL);}}
}  // namespace winrt::Winchisel::implementation
