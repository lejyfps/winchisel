...
winchisel::core::Result<DownloadInstallResult> install_downloads(std::span<winchisel::core::DownloadCatalogEntry const* const> items){DownloadInstallResult result;for(auto item:items){auto end=item->winget_ids.find('|');auto id=item->winget_ids.substr(0,end);if(id.empty()){++result.failed;result.failure_details.push_back(std::string(item->name)+": no winget ID");continue;}auto command=run_hidden(L"winget.exe install --id \""+wide(id)+L"\" --exact --accept-package-agreements --accept-source-agreements --disable-interactivity");if(command&&command->exit_code==0)++result.succeeded;else{++result.failed;result.failure_details.push_back(std::string(item->name)+": "+(command?"winget exit "+std::to_string(command->exit_code):command.error().detail));}}{std::scoped_lock lock(cache_mutex);cache_valid=false;}return result;}
