...
winchisel::core::Result<void> open_https_url(std::wstring_view url) {
    if (!url.starts_with(L"https://")) {
        return std::unexpected(winchisel::core::Error{winchisel::core::ErrorCode::parse, "invalid_url", "Only HTTPS URLs are allowed"});
    }
    ...
    if (result <= 32) {
        return std::unexpected(winchisel::core::Error{winchisel::core::ErrorCode::platform, "shell_execute_failed", std::to_string(result)});
    }
    return {};
}
